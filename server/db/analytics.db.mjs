...
function normalizeAuxAnalyticsProviderId(pidRaw) {
 ...
 /* if (p.startsWith("gemini")) return "gemini-flash";
  ...
  if (p === "or-1" || p.startsWith("kimi")) return "ollama-kimi";
  if (p === "or-2" || p.startsWith("deepseek")) return "ollama-ds";
  if (p === "or-3" || p.includes("openrouter")) return "openrouter";
...
import { openRouterModelPriceMap } from "./openrouterPrices.mjs";
 ...
 */
function estimateProviderUsd(providerId, promptTokens, completionTokens, modelId) {
  const p = Number(promptTokens) || 0;
  const c = Number(completionTokens) || 0;

  // Precise per-model pricing for OpenRouter
  if ( (providerId === "or-1" || providerId === "or-2" || providerId === "or-3") && modelId) {
    const price = openRouterModelPriceMap.get(String(modelId));
    if (price) {
  ...
  const inputUsd = (p / 1_000_000) * rate.input;
  const outputUsd = (c / 1_000_000) * rate.output;
  return { inputUsd, outputUsd, totalUsd: inputUsd + outputUsd };
}
...
async function getAnalyticsPayload() {
  ...
  // ── OpenRouter per-model cost breakdown ────────────────────────────────────
  let openRouterPerModelInputUsd  = 0;
  let openRouterPerModelOutputUsd = 0;
  let openRouterPerModelTotalUsd  = 0;

  if (openRouterModelPriceMap.size > 0) {
    const orRows = await adapter.all(
      `SELECT
         COALESCE(NULLIF(TRIM(t.responding_model_id), ''), '') AS model_id,
         SUM(COALESCE(t.llm_prompt_tokens, 0))     AS tokens_prompt,
         SUM(COALESCE(t.llm_completion_tokens, 0)) AS tokens_completion
       FROM conversation_turns t
       INNER JOIN dialogs d ON d.id = t.dialog_id
       WHERE ${analyticsDialogWhereSql("d")}
         AND (COALESCE(NULLIF(TRIM(t.responding_provider_id), ''), t.requested_provider_id)) = 'openrouter'
       GROUP BY model_id`,
    );
    for (const row of orRows) {
      const mid = String(row.model_id ?? "").trim();
      if (!mid) continue;
      const p_ = Number(row.tokens_prompt)      || 0;
      const c_ = Number(row.tokens_completion)  || 0;
      const est = estimateProviderUsd("openrouter", p_, c_, mid);
      openRouterPerModelInputUsd  += est.inputUsd;
      openRouterPerModelOutputUsd += est.outputUsd;
      openRouterPerModelTotalUsd  += est.totalUsd;
    }
  }
  ...
  for (const id of ANALYTICS_PROVIDER_IDS) {
    ...
    if (id === "openrouter" && openRouterModelPriceMap.size > 0) {
      spendSummary.inputUsd.total    += openRouterPerModelInputUsd;
      spendSummary.outputUsd.total   += openRouterPerModelOutputUsd;
      spendSummary.combinedUsd.total += openRouterPerModelTotalUsd;
      continue;
    }
